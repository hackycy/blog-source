---
title: Docker入门（六）——Docker Compose
date: 2019-12-11 11:43:25
tags:
    - Docker
categories:
    - Docker
...
`Dockerfile` 模板文件可以让用户很方便的定义一个单独的应用容器。但是在平常使用中，经常会碰到需要多个容器相互配合来完成某项任务的情况。例如要实现一个 Web 项目，除了 Web 服务容器本身，往往还需要再加上后端的数据库服务容器，甚至还包括负载均衡容器等。
